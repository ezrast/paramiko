...
class AuthHandler(object):
    ...
    def _request_auth(self):
        """
        If this is our first time trying to authenticate, sends an
        ssh-userauth service request first, and the actual auth info
        will be sent later by _parse_service_accept().
        Otherwise, just calls _make_auth_request().
        """
...
""".format(
    ...
    def _parse_userauth_failure(self, m):
        authlist = m.get_list()
        self.partial_auth = m.get_boolean()
        if self.partial_auth:
